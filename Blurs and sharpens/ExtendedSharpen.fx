// @Maintainer jwrl
// @Released 2026-06-15
// @Author Jerker
// @Author khaver
// @Author jwrl
// @Created 2017-06-19

/**
 Originally released here as unsharp mask, this just that - a simple unsharp mask.  It
 may appear redundant, since the Lightworks effect does pretty much the same thing,
 but this one has a much expanded range and control of edge gain and contrast.  I've
 included it because I like the look of it, and because of that wider range.

   [*]Sharpen zone:  This adjusts the area around which edge detection will take place.
   [*]Threshold:  The value beyond which the effect will not attempt to sharpen the image.
   [*]Edge gain:  Pretty self explanatory. Adjusts the gain of the sharpened edge.
   [*]Edge contrast:  As above, but contrast.
   [*]Amount:  The amount of sharpened image that will be mixed back into the original.

 NOTE:  This effect is not suitable for use with versions of Lightworks earlier than
 2023.1.  It will also break resolution independence.
*/

//-----------------------------------------------------------------------------------------//
// Lightworks user effect ExtendedSharpen.fx
//
// *********************************** ORIGINAL HEADER **************************************
//
// Unsharp Mask by Jerker (Sound and Vision Unit) - was based on Big Blur by khaver, and
// borrowed the main blur algorithm but simplified it by removing the red, green and blue
// color settings.  See http://software.soundandvision.se
//
// Original description: Big Blur by khaver
//
// Smooth blur using a 12 tap circular kernel that rotates 5 degrees for each of 6 passes
// There's a checkbox for a 10 fold increase in the blur amount.  (This was removed and
// fixed at 5 in Jerker's effect.  He also had a bug in his version of the code that meant
// that although he included the six passes that khaver did, his code could only use five
// of them. That bug has been fixed - jwrl.)
//
// Further notes by jwrl.
//
// The effect was rewritten 19 July 2017.  Assumptions had been made about the way that
// shaders functioned in Lightworks that at best could only be described as coincidental
// if they were even true at all.  The original chromatic unsharpen shader was discarded
// altogether and instead the sharpening now just uses luminance.  Edge gain and contrast
// have also been provided, which were not available in the original effect.
//
// ******************************** END OF ORIGINAL HEADER **********************************
//
// Version history:
//
// Updated 2026-06-15 jwrl.
// Changed masking from R to RGBA.
// Changed "Sharpen radius" to "Sharpen zone"
// Added settings description to header text.
//
// Rewrite 2024-04-18 jwrl.
// Corrected a bug that meant that the original effect could not handle negative values
// of edge enhancement.
// Inverted the sense of the threshold adjustment so that the threshold expanded as the
// parameter was increased.  This is technically wrong but from a user's point of view
// it's easier to understand.
// For the same reason changed unsharp radius to sharpen radius.
// Renamed the effect because the changes were significant enough to warrant it.
//-----------------------------------------------------------------------------------------//

#include "_utils.fx"

DeclareLightworksEffect ("Extended sharpen", "Stylize", "Blurs and sharpens", "If the Lightworks sharpen effects don't have enough range, try this.", kNoFlags);

//-----------------------------------------------------------------------------------------//
// Inputs
//-----------------------------------------------------------------------------------------//

DeclareInput (Input);

DeclareMask;

//-----------------------------------------------------------------------------------------//
// Parameters
//-----------------------------------------------------------------------------------------//

DeclareFloatParam (SharpenRadius, "Sharpen zone",  kNoGroup, kNoFlags, 0.2, 0.0, 1.0);
DeclareFloatParam (Threshold,     "Threshold",     kNoGroup, kNoFlags, 0.5, 0.0, 1.0);
DeclareFloatParam (EdgeGain,      "Edge gain",     kNoGroup, kNoFlags, 0.5, 0.0, 1.0);
DeclareFloatParam (EdgeGamma,     "Edge contrast", kNoGroup, kNoFlags, 0.5, 0.0, 1.0);
DeclareFloatParam (Amount,        "Amount",        kNoGroup, kNoFlags, 0.15, 0.0, 1.0);

DeclareFloatParam (_OutputWidth);
DeclareFloatParam (_OutputHeight);

//-----------------------------------------------------------------------------------------//
// Definitions and declarations
//-----------------------------------------------------------------------------------------//

#define LUMA_DOT  float3(1.1955,2.3464,0.4581)
#define GAMMA_VAL 1.666666667

//-----------------------------------------------------------------------------------------//
// Functions
//-----------------------------------------------------------------------------------------//

float4 fn_blur (sampler B, float2 uv, float angle)
{  
   float4 orig = tex2D (B, uv);

   if (SharpenRadius <= 0.0) return orig;

   float radius = SharpenRadius * 100.0;

   float2 pixsize = 1.0.xx / float2 (_OutputWidth, _OutputHeight);
   float2 halfpix = pixsize / 2.0;
   float2 xy2, xy1 = uv + halfpix;

   float4 cOut = tex2D (B, xy1);

   for (int tap = 0; tap < 12; tap++) {
      sincos (angle, xy2.y, xy2.x);                             
      xy1 = uv + (halfpix * xy2 * radius);

      cOut += tex2D (B, xy1);
      angle += 0.5236;
   }

   cOut /= 13.0;

   return cOut;
}

//-----------------------------------------------------------------------------------------//
// Code
//-----------------------------------------------------------------------------------------//

// technique UnsharpMask

// This pass maps the foreground clip to TEXCOORD2, so that variations in clip
// geometry and rotation without needing to be handled inside the blur loops.

DeclarePass (Inp)
{ return ReadPixel (Input, uv1); }

DeclarePass (Pass1)
{ return fn_blur (Inp, uv2, 0.0); }

DeclarePass (Pass2)
{ return fn_blur (Pass1, uv2, 0.0873); }

DeclarePass (Pass3)
{ return fn_blur (Pass2, uv2, 0.1745); }

DeclarePass (Pass4)
{ return fn_blur (Pass3, uv2, 0.2618); }

DeclarePass (Pass5)
{ return fn_blur (Pass4, uv2, 0.3491); }

DeclarePass (Blur)
{ return fn_blur (Pass5, uv2, 0.4364); }

DeclareEntryPoint (UnsharpMask)
{
   if (IsOutOfBounds (uv1)) return kTransparentBlack;

   float4 retval = tex2D (Inp, uv2);

   if (Amount <= 0.0) return retval;

   float maskLimit = 1.0 - Threshold;
   float maskGamma = min (1.15, 1.1 - min (1.05, EdgeGamma)) * GAMMA_VAL;
   float maskGain  = EdgeGain * 2.0;

   maskGamma *= maskGamma;

   float sharpen_1 = dot (tex2D (Blur, uv2).rgb, LUMA_DOT);
   float sharpen_2 = dot (retval.rgb, LUMA_DOT);
   float sharpen_3 = pow (max (0.0, sharpen_2 - sharpen_1 - maskLimit), maskGamma);
   float sharpen_4 = pow (max (0.0, sharpen_1 - sharpen_2 - maskLimit), maskGamma);

   float4 sharp  = float4 (retval.rgb + (sharpen_3 - sharpen_4) * maskGain.xxx, retval.a);

   float4 result = lerp (retval, sharp, Amount);

   return lerp (retval, result, tex2D (Mask, uv2));
}
// @Maintainer jwrl
// @Released 2024-04-18
// @Author Jerker
// @Author khaver
// @Author jwrl
// @Created 2017-06-19

/**
 Originally released here as unsharp mask, this just that - a simple unsharp mask.  It
 may appear redundant, since the Lightworks effect does pretty much the same thing,
 but this one has a much expanded range and control of edge gain and contrast.  I've
 included it because I like the look of it, and because of that wider range.

 NOTE:  This effect is not suitable for use with versions of Lightworks earlier than
 2023.1.  It will also break resolution independence.
*/

//-----------------------------------------------------------------------------------------//
// Lightworks user effect ExtendedSharpen.fx
//
// *********************************** ORIGINAL HEADER **************************************
//
// Unsharp Mask by Jerker (Sound and Vision Unit) - was based on Big Blur by khaver, and
// borrowed the main blur algorithm but simplified it by removing the red, green and blue
// color settings.  See http://software.soundandvision.se
//
// Original description: Big Blur by khaver
//
// Smooth blur using a 12 tap circular kernel that rotates 5 degrees for each of 6 passes
// There's a checkbox for a 10 fold increase in the blur amount.  (This was removed and
// fixed at 5 in Jerker's effect.  He also had a bug in his version of the code that meant
// that although he included the six passes that khaver did, his code could only use five
// of them. That bug has been fixed - jwrl.)
//
// Further notes by jwrl.
//
// The effect was rewritten 19 July 2017.  Assumptions had been made about the way that
// shaders functioned in Lightworks that at best could only be described as coincidental
// if they were even true at all.  The original chromatic unsharpen shader was discarded
// altogether and instead the sharpening now just uses luminance.  Edge gain and contrast
// have also been provided, which were not available in the original effect.
//
// ******************************** END OF ORIGINAL HEADER **********************************
//
// Version history:
//
// Rewrite 2024-04-18 jwrl.
// Corrected a bug that meant that the original effect could not handle negative values
// of edge enhancement.
// Inverted the sense of the threshold adjustment so that the threshold expanded as the
// parameter was increased.  This is technically wrong but from a user's point of view
// it's easier to understand.
// For the same reason changed unsharp radius to sharpen radius.
// Renamed the effect because the changes were significant enough to warrant it.
//-----------------------------------------------------------------------------------------//

#include "_utils.fx"

DeclareLightworksEffect ("Extended sharpen", "Stylize", "Blurs and sharpens", "If the Lightworks sharpen effects don't have enough range, try this.", kNoFlags);

//-----------------------------------------------------------------------------------------//
// Inputs
//-----------------------------------------------------------------------------------------//

DeclareInput (Input);

DeclareMask;

//-----------------------------------------------------------------------------------------//
// Parameters
//-----------------------------------------------------------------------------------------//

DeclareFloatParam (SharpenRadius, "Sharpen radius", kNoGroup, kNoFlags, 0.2, 0.0, 1.0);
DeclareFloatParam (Threshold, "Threshold", kNoGroup, kNoFlags, 0.5, 0.0, 1.0);
DeclareFloatParam (EdgeGain, "Edge gain", kNoGroup, kNoFlags, 0.5, 0.0, 1.0);
DeclareFloatParam (EdgeGamma, "Edge contrast", kNoGroup, kNoFlags, 0.5, 0.0, 1.0);
DeclareFloatParam (Amount, "Amount", kNoGroup, kNoFlags, 0.15, 0.0, 1.0);

DeclareFloatParam (_OutputWidth);
DeclareFloatParam (_OutputHeight);

//-----------------------------------------------------------------------------------------//
// Definitions and declarations
//-----------------------------------------------------------------------------------------//

#define LUMA_DOT  float3(1.1955,2.3464,0.4581)
#define GAMMA_VAL 1.666666667

//-----------------------------------------------------------------------------------------//
// Functions
//-----------------------------------------------------------------------------------------//

float4 fn_blur (sampler B, float2 uv, float angle)
{  
   float4 orig = tex2D (B, uv);

   if (SharpenRadius <= 0.0) return orig;

   float radius = SharpenRadius * 100.0;

   float2 pixsize = 1.0.xx / float2 (_OutputWidth, _OutputHeight);
   float2 halfpix = pixsize / 2.0;
   float2 xy2, xy1 = uv + halfpix;

   float4 cOut = tex2D (B, xy1);

   for (int tap = 0; tap < 12; tap++) {
      sincos (angle, xy2.y, xy2.x);                             
      xy1 = uv + (halfpix * xy2 * radius);

      cOut += tex2D (B, xy1);
      angle += 0.5236;
   }

   cOut /= 13.0;

   return cOut;
}

//-----------------------------------------------------------------------------------------//
// Code
//-----------------------------------------------------------------------------------------//

// technique UnsharpMask

// This pass maps the foreground clip to TEXCOORD2, so that variations in clip
// geometry and rotation without needing to be handled inside the blur loops.

DeclarePass (Inp)
{ return ReadPixel (Input, uv1); }

DeclarePass (Pass1)
{ return fn_blur (Inp, uv2, 0.0); }

DeclarePass (Pass2)
{ return fn_blur (Pass1, uv2, 0.0873); }

DeclarePass (Pass3)
{ return fn_blur (Pass2, uv2, 0.1745); }

DeclarePass (Pass4)
{ return fn_blur (Pass3, uv2, 0.2618); }

DeclarePass (Pass5)
{ return fn_blur (Pass4, uv2, 0.3491); }

DeclarePass (Blur)
{ return fn_blur (Pass5, uv2, 0.4364); }

DeclareEntryPoint (UnsharpMask)
{
   if (IsOutOfBounds (uv1)) return kTransparentBlack;

   float4 retval = tex2D (Inp, uv2);

   if (Amount <= 0.0) return retval;

   float maskLimit = 1.0 - Threshold;
   float maskGamma = min (1.15, 1.1 - min (1.05, EdgeGamma)) * GAMMA_VAL;
   float maskGain  = EdgeGain * 2.0;

   maskGamma *= maskGamma;

   float sharpen_1 = dot (tex2D (Blur, uv2).rgb, LUMA_DOT);
   float sharpen_2 = dot (retval.rgb, LUMA_DOT);
   float sharpen_3 = pow (max (0.0, sharpen_2 - sharpen_1 - maskLimit), maskGamma);
   float sharpen_4 = pow (max (0.0, sharpen_1 - sharpen_2 - maskLimit), maskGamma);

   float4 sharp  = float4 (retval.rgb + (sharpen_3 - sharpen_4) * maskGain.xxx, retval.a);

   float4 result = lerp (retval, sharp, Amount);

   return lerp (retval, result, tex2D (Mask, uv2).x);
}
