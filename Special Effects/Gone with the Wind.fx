...
/**
 ...
 away.  It's a very nice effect.  The parameters and what they do are:
   ...
   [*]Wind direction
      ...
      [*]Spread: Sets the angular spread of the granular pattern..
   [*]Fg blend modes: 0% is Normal, 100% is Screen
...
// Version history:
//
// Modified 2026-07-06 jwrl.
// Changed file name from "Gone with the Wind 20240502.fx" to "Gone with the Wind".
// Changed "Spread angle" to "Spread", displaying it as degrees.
// Masking now uses RGBA.
//
// Modified 2024-09-03 jwrl.
// Added grain size adjustment.
//
...
DeclareEntryPoint (Main)
{
   ...
   destruction *=  (1.0 - Amount);                  // Ensures maximum destruction when Amount==1.
                                                    //  Warning, it can be negative!
   ...
   // Now create an angle dependent dither radius

   float2 ditherCoord = fn_ditherCoord (uv1, Amount * Amount * 10.0, noise.x, randomRadius);

   // Now get the Mask for dither source selection (inside the mask == 1, outside == 0)

   float4 fgMaskDith = tex2D (Mask, ditherCoord);
   float4 fgMask     = tex2D (Mask, uv1);           // Mask related to the original texture
   ...
   // Fade out the dithered foreground  Â´fadeÂ´ range 0 ... 1 if the Â´AmountÂ´ range <= 0.95 ... 1

   float fade = saturate ((Amount - 0.95) * 20.0);

   // Fade out pixels remaining in the texture (in the amount range of 0.95 ...1)

   fgDith.a *= 1.0 - fade;

   // Mask-dependent foreground transparencies ...

   fgDith *= fgMaskDith;                           // Replaces areas outside the mask with transparent black.
   fg *= 1.0.xxxx - fgMask;                        // Areas that correspond to the areas outside the mask become transparent.
  ...
  // Merge both foreground samples ...
   ...
   // Choosing or mixing foreground transparent modes.

   return float4 (lerp (mix1, mix2, BlendMode).rgb, 1.0);
}
