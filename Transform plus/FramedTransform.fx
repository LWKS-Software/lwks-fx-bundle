...
/**
 ...
 final result as a single transform effect would.
   ...
   [*]Crop left X:  Crops the left of the beveled foreground.
   [*]Crop top Y:  Crops the top of the beveled foreground.
   [*]Crop right:  Crops the right of the beveled foreground.
   [*]Crop bottom:  Crops the bottom of the beveled foreground.
   [*]Video insert
   ...
   [*]Border
      ...
      [*]Bevel width:  Adjusts the bevel width without changing the overall
         border size.
      [*]Sharpness:  Reducing the bevel sharpness increases the apparent
         roundness.
      [*]Outer level:  Adjusts the brightness of the outer bevel edge.
      [*]Inner level:  Adjusts the brightness of the inner bevel edge.
      [*]Texture size:  Scales the texture used by the border.
...
// Cropping is now directly addressed with no drag capability.
// Changed "Bevel sharpness" to "Sharpness".
// Changed "Outer brightness" to "Outer level".
// Changed "Inner brightness" to "Inner level".
// Changed "Texture scale" to "Texture size"
//
...
DeclareFloatParam (TLcropX,        "Crop left",    kNoGroup,       "SpecifiesPointX", 0.1, 0.0, 1.0);
DeclareFloatParam (TLcropY,        "Crop top",     kNoGroup,       "SpecifiesPointY", 0.9, 0.0, 1.0);
DeclareFloatParam (BRcropX,        "Crop right",   kNoGroup,       "SpecifiesPointX", 0.9, 0.0, 1.0);
DeclareFloatParam (BRcropY,        "Crop bottom",  kNoGroup,       "SpecifiesPointY", 0.1, 0.0, 1.0);
...
DeclareFloatParam (BevelSharpness, "Sharpness",    "Border",       kNoFlags,  0.2,  0.0, 1.0);
DeclareFloatParam (BevelOuter,     "Outer level",  "Border",       kNoFlags,  0.6, -1.0, 1.0);
DeclareFloatParam (BevelInner,     "Inner level",  "Border",       kNoFlags, -0.4, -1.0, 1.0);
DeclareFloatParam (TextureScale,   "Texture size", "Border",       kNoFlags,  1.0,  0.5, 2.0);
...
DeclareEntryPoint (FramedTransform)
{
   ...
   return lerp (Bgnd, retval, tex2D (Mask, uv4) * Opacity);
}
